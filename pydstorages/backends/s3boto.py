...
class S3BotoStorageFile(File):
    ...
    buffer_size = settings.get('AWS_S3_FILE_BUFFER_SIZE', 5242880)
...
class S3BotoStorage(Storage):
    ...
    access_key = settings.get('AWS_S3_ACCESS_KEY_ID', settings.get('AWS_ACCESS_KEY_ID'))
    secret_key = settings.get('AWS_S3_SECRET_ACCESS_KEY', settings.get('AWS_SECRET_ACCESS_KEY'))
    file_overwrite = settings.get('AWS_S3_FILE_OVERWRITE', True)
    headers = settings.get('AWS_HEADERS', {})
    bucket_name = settings.get('AWS_STORAGE_BUCKET_NAME')
    auto_create_bucket = settings.get('AWS_AUTO_CREATE_BUCKET', False)
    default_acl = settings.get('AWS_DEFAULT_ACL', 'public-read')
    bucket_acl = settings.get('AWS_BUCKET_ACL', default_acl)
    querystring_auth = settings.get('AWS_QUERYSTRING_AUTH', True)
    querystring_expire = settings.get('AWS_QUERYSTRING_EXPIRE', 3600)
    reduced_redundancy = settings.get('AWS_REDUCED_REDUNDANCY', False)
    location = settings.get('AWS_LOCATION', '')
    encryption = settings.get('AWS_S3_ENCRYPTION', False)
    custom_domain = settings.get('AWS_S3_CUSTOM_DOMAIN')
    calling_format = settings.get('AWS_S3_CALLING_FORMAT', SubdomainCallingFormat())
    secure_urls = settings.get('AWS_S3_SECURE_URLS', True)
    file_name_charset = settings.get('AWS_S3_FILE_NAME_CHARSET', 'utf-8')
    gzip = settings.get('AWS_IS_GZIPPED', False)
    preload_metadata = settings.get('AWS_PRELOAD_METADATA', False)
    gzip_content_types = settings.get('GZIP_CONTENT_TYPES', (
        'text/css',
        'application/javascript',
        'application/x-javascript',
    ))
    url_protocol = settings.get('AWS_S3_URL_PROTOCOL', 'http:')
    host = settings.get('AWS_S3_HOST', S3Connection.DefaultHost)
    use_ssl = settings.get('AWS_S3_USE_SSL', True)
    port = settings.get('AWS_S3_PORT', None)
